...
**Tanggal Dokumen Diperbarui:** 19 Mei 2026
...
Proyek SupplierHub mengadopsi arsitektur *Full-Stack* yang terdiri dari dua direktori utama: `frontend/` dan `backend/`.
...
### Folder `backend/` (Golang Fiber)
- `main.go`: *File* utama server yang mendefinisikan *endpoint* API (Login, Products, Orders, Stats) serta konfigurasi CORS. Memanggil *package* `database` dan `models`.
- `database/database.go`: Konfigurasi koneksi ke basis data MySQL (Laragon) menggunakan GORM.
- `models/models.go`: Definisi struktur tabel (*schema*) basis data menggunakan *struct* Golang (seperti tabel `User`, `Product`, dan `Order`).
...
- **Autentikasi (API-Based Login)**
  Login tidak lagi simulasi. Kredensial divalidasi langsung oleh basis data MySQL, mengembalikan data profil pengguna untuk manajemen akses (Admin Supplier vs User UMKM).
...
Aplikasi telah bertransformasi menjadi platform yang komprehensif dengan *tech stack* terbaru:
...
**Backend:**
- **Golang (Go)**: Bahasa pemrograman kompilasi berkinerja tinggi.
- **Fiber**: *Web framework* Golang yang sangat cepat, terinspirasi dari Express.js.
- **GORM & MySQL**: ORM untuk manajemen basis data relasional (menggantikan SQLite).

---

## 5. Rencana Pengembangan Kedepannya

- **Sistem Keamanan Berbasis Token (JWT)**: Menggantikan pengiriman data kredensial *plaintext* menjadi autentikasi token JWT guna keamanan *session* tingkat lanjut.
- **Fitur Keranjang (*Cart*)**: Memungkinkan pengguna UMKM untuk menambahkan beberapa produk berbeda sekaligus ke dalam keranjang sebelum melakukan *checkout* pesanan.
...
- **Integrasi Pembayaran (Payment Gateway)**: Menambahkan modul pembayaran pihak ketiga untuk otomatisasi verifikasi transaksi.
